...
export class RxjsExample10Component {
    ...
    private readonly mouseUp$ = fromEvent<MouseEvent>(this.documentRef, 'mouseup').pipe(tap(() => console.log('mouseup')));
    ...
    readonly coordinates$ = this.mouseDown$.pipe(
        switchMap(down =>
            this.mouseMove$.pipe(
                ...
                map(([x, y]) => [clamp(x, 0, PALETTE_WIDTH), clamp(y, 0, PALETTE_HEIGHT)]),
    ...
    readonly hsv$: Observable<[number, number, number]> = this.coordinates$.pipe(
        map(coordinates => [HUE, coordinates[0] / PALETTE_WIDTH, ((PALETTE_HEIGHT - coordinates[1]) / PALETTE_HEIGHT) * 255])
    );
