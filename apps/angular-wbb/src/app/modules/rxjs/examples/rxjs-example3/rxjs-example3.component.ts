...
import { catchError, ignoreElements, mapTo, repeat, retry, share, startWith, switchMap, switchMapTo } from 'rxjs/operators';
import { animate, style, transition, trigger } from '@angular/animations';
import { LoginService } from "./login.service";

export const COLLAPSE = trigger('collapse', [
    transition(':enter', [style({ height: 0, opacity: 0 }), animate('.2s', style({ height: '*', opacity: 1 }))]),
    transition(':leave', [style({ height: '*' }), animate('.2s', style({ height: 0, opacity: 0 }))])
]);
...
export class RxjsExample3Component implements OnInit {
    ...
    public readonly request$ = this.submit$.pipe(switchMapTo(this.service.pipe(startWith(''))), share());
