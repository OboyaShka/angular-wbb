...
import { BehaviorSubject, combineLatest, concat, fromEvent, merge, Observable, of, Subject } from 'rxjs';
import {
...
export function dragAndDropFrom(element: Element): Observable<DragState> {
    ...
    return concat(
        ...
        merge(
            fromEvent<MouseEvent>(ownerDocument, 'mousemove').pipe(map(event => new DragState(DragStage.Continues, event))),
            merge(fromEvent<MouseEvent>(ownerDocument, 'mouseup'), fromEvent<MouseEvent>(ownerDocument, 'dragend')).pipe(
                first(),
...
export class RxjsExample16Component {
    ...
    readonly wrapperTranslate$ = combineLatest([this.drag$.pipe(startWith(null), pairwise()), this.zoom$, this.rotation$]).pipe(
        scan<[[DragState | null, DragState | null], number, number], [number, number]>((coordinates, latest) => {
    ...
    readonly transitioned$ = merge(
        this.drag$.pipe(
            ...
            switchMap(() =>
                merge(this.zoom$.pipe(filter(zoom => zoom !== DEFAULT_ZOOM)), this.rotation$.pipe(filter(rotation => rotation !== 0))).pipe(
                    mapTo(true),
                    startWith(false)
                )
            )
    ...
    readonly wrapperTransform$ = combineLatest([this.wrapperTranslate$, this.zoom$, this.rotation$]).pipe(
        map(([translate, zoom, rotation]) => `translate(${translate}) scale(${zoom}) rotate(${rotation}deg)`)
    );
